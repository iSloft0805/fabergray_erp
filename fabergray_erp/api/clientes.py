# -*- coding: utf-8 -*-
"""Commit 22.1 -- read-only API layer for the future Page Clientes (Commit
22.3), used by the new "Gestión de Clientes" role (this commit's own setup:
Role + Custom DocPerm on Customer -- read/write/create, no delete --
versioned in fixtures/role.json + custom_docperm.json, the same mechanism
already established for Bodega/Jefe de Bodega/Vendedora/Facturación).
Vendedora's own existing Customer/Address/Contact read permission is
untouched -- this is a new, separate role, not an extension of hers.

This module itself never writes: every function below is
frappe.get_list()/frappe.get_doc() only, never insert()/save(). Write
endpoints (create/edit/activar-desactivar) are explicitly out of scope --
deferred to Commit 22.2.

Same conventions as api/ventas.py/bodega.py/facturacion.py: every read
goes through frappe.get_list()/frappe.get_doc()+check_permission(), never
frappe.get_all() (which bypasses permissions), never
ignore_permissions=True, never frappe.set_user() outside of tests.

Scope, exactly as approved in the Fase 2 audit: Customer only (native
doctype -- no custom Client doctype). Address/Contact are surfaced only as
far as the primary links Customer itself already stores
(customer_primary_contact/customer_primary_address) -- no Address/Contact
list or CRUD here, deferred as approved."""
